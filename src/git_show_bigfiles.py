...
JSONItem = Union[str, int, float, bool, Date,
                 Time, None, Dict[str, Any], List[Any]]
...
def tabToFMT(fmt: str, result: JSONList, sorts: RowSortList = [], formats: Dict[str, str] = {}, *,  #
    ...
    lines: List[str] = []
    if not noheaders:
        line = [rightF(name, tab2 + "%%-%is" % cols[name]) %
                name for name in sorted(cols.keys(), key=sortkey)]
...
def _main_() -> int:
    ...
    if opt.logfile:
        ...
        _logfile.setFormatter(logging.Formatter(
